...
export class MessagingService {
  ...
  createRequest(dto: { patientId: number; doctorId: number }): Observable<MessageRequest> {
    return this.http.post<MessageRequest>(`${this.apiUrl}/request`, dto);
  }

  getRequests(doctorId: number): Observable<MessageRequest[]> {
    return this.http.get<MessageRequest[]>(`${this.apiUrl}/requests/${doctorId}`);
  }

  updateRequestStatus(requestId: number, statut: string): Observable<MessageRequest> {
    return this.http.put<MessageRequest>(`${this.apiUrl}/requests/${requestId}/status`, { statut });
  }

  createOrFetchConversation(dto: { patientId: number; doctorId: number; dateDebut: string; messages: any[] }): Observable<Conversation> {
  ...
  markMessageAsSeen(messageId: number): Observable<Message> {
    return this.http.put<Message>(`${this.apiUrl}/messages/${messageId}/seen`, {});
  }
}
