...
export class ApiService {
  ...
  // Patient endpoints
  getPatients(filtres: FiltreRecherche): Observable<ApiResponse<Pagination<Patient>>> {
    const params = this.buildQueryParams(filtres);
    return this.http.get<ApiResponse<Pagination<Patient>>>(`${this.apiUrl}/api/patients`, { params });
  }

  getPatient(id: number): Observable<ApiResponse<Patient>> {
    return this.http.get<ApiResponse<Patient>>(`${this.apiUrl}/api/patients/${id}`);
  }

  createPatient(patient: Patient): Observable<ApiResponse<Patient>> {
    return this.http.post<ApiResponse<Patient>>(`${this.apiUrl}/api/patients`, patient);
  }

  updatePatient(id: number, patient: Patient): Observable<ApiResponse<Patient>> {
    return this.http.put<ApiResponse<Patient>>(`${this.apiUrl}/api/patients/${id}`, patient);
  }

  deletePatient(id: number): Observable<ApiResponse<void>> {
    return this.http.delete<ApiResponse<void>>(`${this.apiUrl}/api/patients/${id}`);
  }

  // Surveillance endpoints
  getConstantesVitales(patientId: number): Observable<ApiResponse<ConstantesVitales>> {
  ...
  // Chambre endpoints
  getChambres(filtres?: FiltreRecherche): Observable<ApiResponse<Pagination<Chambre>>> {
    const params = this.buildQueryParams(filtres);
    return this.http.get<ApiResponse<Pagination<Chambre>>>(`${this.apiUrl}/api/chambres`, { params });
  }

  // Personnel endpoints
  getPersonnel(filtres?: FiltreRecherche): Observable<ApiResponse<Pagination<Personnel>>> {
    const params = this.buildQueryParams(filtres);
    return this.http.get<ApiResponse<Pagination<Personnel>>>(`${this.apiUrl}/api/personnel`, { params });
  }

  // Rendez-vous endpoints
  createRendezVous(rendezvous: Rendezvous): Observable<ApiResponse<Rendezvous>> {
    return this.http.post<ApiResponse<Rendezvous>>(`${this.apiUrl}/api/rendez-vous`, rendezvous);
  }
  ...
  private buildQueryParams(filtres?: FiltreRecherche): HttpParams {
    ...
    if (filtres.pageSize) params = params.set('size', filtres.pageSize.toString());
    if (filtres.tri) {
